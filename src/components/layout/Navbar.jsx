            ...
            <div className="p-6 lg:p-8 pb-4 flex justify-between items-center border-b border-gray-100">
               <div className="flex items-center gap-2">
                  ...
                  <span className="text-xl font-bold text-emerald-950">
                    Himachal<span className="text-orange-500">Destination</span>
                  </span>
               </div>
               <button onClick={() => setIsSidebarOpen(false)} className="w-10 h-10 rounded-full border border-gray-200 flex items-center justify-center text-gray-500 hover:bg-orange-500 hover:text-white hover:border-orange-500 transition-all">
                 <X className="w-5 h-5" />
               </button>
            </div>
            ...
            <div className="p-6 lg:p-8 space-y-10">
              ...
              <div>
                <div className="flex justify-between items-end mb-6">
                  ...
                  <Link 
                    to="/Destination" 
                    onClick={() => setIsSidebarOpen(false)}
                    className="text-xs font-bold text-orange-500 hover:text-orange-600 flex items-center gap-1 group/view bg-orange-50 px-3 py-1.5 rounded-full transition-colors"
                  >
                    View All <ArrowRight className="w-3.5 h-3.5 transition-transform group-hover/view:translate-x-1" />
                  </Link>
                </div>
                ...
                <div className="relative h-64 rounded-2xl overflow-hidden shadow-lg group cursor-pointer">
                  {sidebarDestinations.map((dest, index) => (
                    ...
                    >
                      <img 
                        src={dest.image} 
                        alt={dest.name} 
                        className="w-full h-full object-cover transition-transform duration-[2000ms] scale-105 group-hover:scale-110"
                      />
                      <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent"></div>
                      
                      <div className="absolute top-4 right-4 bg-orange-500 text-white text-[10px] font-bold px-3 py-1 rounded shadow-md">
                        {dest.tours} Tours
                      </div>
                  ...
                  <div className="absolute bottom-4 right-4 z-20 flex gap-2">
                    {sidebarDestinations.map((_, index) => (
                      <span 
                        key={index} 
                        className={`h-1.5 rounded-full transition-all duration-500 ${index === currentDestIndex ? 'w-6 bg-orange-500' : 'w-1.5 bg-white/50'}`}
                      />
                    ))}
                  </div>
                </div>
              ...
              <div className="pt-6 border-t border-gray-100">
                  <p className="text-gray-500 text-sm leading-relaxed mb-4">
                    We provide the best travel experiences in Himachal Pradesh with personalized tour packages.
                  </p>
                  <div className="flex items-center gap-3">
                    <Phone className="w-5 h-5 text-orange-500" />
                    <span className="text-lg font-bold text-emerald-950">+91 98765-43210</span>
                  </div>
              </div>
            </div>
          </div>
        ...
        <div className={`absolute right-0 top-0 h-full w-[280px] bg-white shadow-2xl transition-transform duration-300 transform ${isMobileMenuOpen ? "translate-x-0" : "translate-x-full"}`}>
          <div className="p-6 flex flex-col h-full bg-gradient-to-b from-white to-orange-50/30">
            ...
            <div className="flex flex-col gap-2 overflow-y-auto flex-1">
              {navLinks.map((link) => (
                <div key={link.name}>
                  <Link to={link.path} onClick={() => setIsMobileMenuOpen(false)} className={`flex items-center justify-between p-3.5 rounded-xl transition-all ${location.pathname === link.path ? "bg-orange-50 text-orange-600 font-bold border border-orange-100" : "text-emerald-900 font-medium hover:bg-gray-50 hover:text-orange-600"}`}>
                    {link.name} <ArrowRight className={`w-4 h-4 ${location.pathname === link.path ? "opacity-100" : "opacity-30"}`} />
                  </Link>
                  {link.hasDropdown && (
                      <div className="pl-6 flex flex-col gap-2 mt-1 border-l-2 border-orange-100 ml-4">
                        {link.subItems.map((sub) => (
                          <Link key={sub.name} to={sub.path} onClick={() => setIsMobileMenuOpen(false)} className="text-sm text-gray-500 py-1 hover:text-orange-500">{sub.name}</Link>
                        ))}
                      </div>
                  )}
                </div>
            ...
            <div className="mt-auto pt-6 border-t border-gray-100">
               ...
               <div className="text-center text-xs text-emerald-800/60 font-medium">© 2024 Himachal Destination</div>
            </div>
          </div>
        </div>
      </div>
